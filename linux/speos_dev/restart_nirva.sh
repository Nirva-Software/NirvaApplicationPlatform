...
sendMail() {
    rm -f $MAIL
    echo "To: monitor@nirva-systems.com" > $MAIL
    echo "From: monitor@nirva-systems.com" >> $MAIL
    echo "Subject: $1"$'\n' >> $MAIL
    echo -e "$2" >> $MAIL
    echo "" >> $MAIL
    echo "" >> $MAIL
    ...
    echo "" >> $MAIL
    tail -n 50 /home/nirva/log/nirva/Logs/SYSTEM/nvs.log >> $MAIL
    /usr/sbin/ssmtp monitor@nirva-systems.com < $MAIL
    rm -f $MAIL
}
...
nvcc -u log_admin -w log_admin -z "NV_CMD='LOG:SET_OPTIONS' LOG='SYSTEM' SERVICE='SYSTEM' LEVEL='6'"
...
echo "Sleep for 1 minute (time for Nirva server to start again) to update the log to previous level (4)"
sleep 60
nvcc -u log_admin -w log_admin -z "NV_CMD='LOG:SET_OPTIONS' LOG='SYSTEM' SERVICE='SYSTEM' LEVEL='4'"
echo
